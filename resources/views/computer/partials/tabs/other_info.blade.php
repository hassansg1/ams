    ...
    <div class="col-lg-4">
        <div class="mb-3">
            ...
            <select class="form-select form-select-input" name="operating_system"
                ...
                <option value=""></option>
                @foreach(\App\Models\Software::where('software_type', 'operating_system')->get() as $var)
                    <option
                            {{ isset($item) && $var->name == $item->operating_system ? 'selected' : ''  }}
...
<div class="row">
    ...
    <div class="col-lg-4">
        <div class="mb-3">
            <label for="{{ isset($item) ? $item->id:'' }}single_point_of_failure" class="form-label">Single Point of
                Failure</label>
            <select class="form-select select2" name="single_point_of_failure"
                ...
                <option value="1" {{ isset($item) && $item->single_point_of_failure == 1  ? 'selected' : ''}}>Yes
                </option>
                <option value="2" {{ isset($item)  && $item->single_point_of_failure == 2  ? 'selected' : ''}}>No
                </option>
            </select>
        </div>
    </div>
