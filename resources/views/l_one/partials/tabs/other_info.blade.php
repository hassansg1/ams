...
<div class="row">
    <div class="col-lg-4">
        <div class="mb-3">
            <label for="{{ isset($item) ? $item->id:'' }}owner_contact" class="form-label">Asset Owner</label>
            <input type="text"
    ...
    <div class="col-lg-4">
        <div class="mb-3">
            ...
            <select class="form-control select2" id="parent_asset_id" name="parent_asset_id">
                ...
                @foreach(\App\Models\Location::whereIn('type',['computer','network','lone'])->get() as $parent_asset_id)
                    <option value="{{ $parent_asset_id->id }}" {{ isset($item) && $item->parent_asset_id == $parent_asset_id->id  ? 'selected' : ''}}>{{ $parent_asset_id->rec_id }}</option>
                @endforeach
            </select>
        </div>
    </div>
...
<div class="row">
    <div class="col-lg-4">
        <div class="mb-3">
            <label for="{{ isset($item) ? $item->id:'' }}parent_asset_id" class="form-label">Control</label>
