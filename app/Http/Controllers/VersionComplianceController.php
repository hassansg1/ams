...
class VersionComplianceController extends BaseController
{
    ...
    public function index(Request $request, $complianceVersionId)
    {
        ...
        return view($this->route . "/index")
            ->with(['data' => $data, 'items' => $tree, 'standardId' => $version->standard_id, 'request' => $request, 'version' => $complianceVersionId, 'route' => $this->route, 'model_name'=>'ClauseData','heading' => "Compliance for " . ($version->standard->name ?? '') . " - Version : " . $version->name ?? '', 'version_id' => $version->id]);
    }
