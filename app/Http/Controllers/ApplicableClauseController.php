...
//        $data = $this->fetchData($this->model, $request);
        ...
        return view($this->route . "/index")
            ->with(['items' => $items, 'route' => $this->route, 'model_name'=>'StandardClause', 'heading' => $this->heading]);
    }
    ...
    public function getLocationsOfCompliance(Request $request)
    {
        ...
        return response()->json([
            'html' => \view('version_compliance.location_table')->with(['locations' => $locations,'complianceD' => $complianceD, 'version' => $version, 'versionId' => $request->version, 'item_id' => $request->trId])->render(),
            'status' => true
        ]);
    }
